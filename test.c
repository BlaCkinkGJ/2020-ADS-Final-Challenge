...
struct Rect rects[] = {
    {{0, 0, 1, 1}}, // xmin, ymin, xmax, ymax (for 2 dimensional RTree)
    {{0, 0, 2, 2}},
    {{2, 2, 3, 4}},
    {{2, 2, 4, 5}},
};
...
struct Rect search_rect = {
    {0, 0, 1, 1}, // search will find above rects that this one overlaps
};
...
int main(void) {
   ...
   */
  for (i = 0; i < nrects; i++)
    RTreeInsertRect(&rects[i], i + 1, &root,
                    0); // i+1 is rect ID. Note: root can change
  RTreeDeleteRect(&rects[0], 1, &root);
